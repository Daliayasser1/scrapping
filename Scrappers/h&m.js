    ...
    while (nextPageUrl) {
      ...
      for (const itemUrl of productLinks) {
        ...
        const productDetails = await page.evaluate(() => {
          ...
          return { title, price, description, productURL, images, sizes, colors };
        });
        console.log(productDetails);
        allItems.push(productDetails);
      }
...
async function scrapHM(options) {
  ...
  for (const url of urls) {
    ...
    while (nextPageUrl) {
      try {
        await page.goto(nextPageUrl, { timeout: 90000 }); // Set the timeout option here
        ...
        for (const itemUrl of productLinks) {
          await page.goto(itemUrl, { timeout: 30000 }); // Set the timeout option here
          await page.waitForSelector("#product-zoom-container img");
          await autoScroll(page);
        ...
        nextPageUrl = null; // Stop processing this page and move to the next one
      }
    }
      ...
      for (let imgIndex = 0; imgIndex < item.images.length; imgIndex++) {
        ...
        try {
          const response = await axios({
            ...
            method: "GET",
            responseType: "stream",
            timeout: 5000, // Set the axios timeout here
          });
